...
export default function Admin(props: AdminProps) {
    ...
    function handleTagAdd(e) {
        ...
        try {
            tag.create(props.wallet)
                .then((id) => {
                    setTags(old => [...old, tag])
                    ApplicationConfiguration.updateLocalStorage({ ...ac, tags: [...tags, tag] })
                })
    ...
    function createApp() {
        ...
        const app = new Application(ac)

        app.create(props.wallet)
            .then((new_ac) => { setApplicationConfiguration({ ...ac, ...new_ac }) })
            .finally(() => { 
                ReloadApplicationConfiguration()
                setLoading(false) 
            })
    ...
    function destroyApp() {
        ...
        app.destroyApplication(props.wallet)
            .then((new_ac) => { props.updateAppConf({ ...ac, ...new_ac }) })
            .finally(() => { setLoading(false) })
    }
    ...
    function getConf(): any  {
        return {
            ...ps,
            ["algod"]: algod,
            ["indexer"]: indexer,
            ["ipfs"]: ipfs,
            ["application"]: { ...ac, tags: tags }
        }
    }
    function getConfText() {
        return JSON.stringify(getConf(), undefined, 4)
    }
    ...
    return (
        <div className='content application-conf' >
            ...
            <InputGroup fill={true}
                ...
                large={true}
                value={props.seed_amt ? props.seed_amt.toString() : ""}
            />
            <InputGroup fill={true}
                onChange={e => { props.set('max_price', e.target.value) }}
                placeholder="Max Price"
                large={true}
                value={props.max_price ? props.max_price.toString() : ""}
            />
            <Button
...
function TagCreator(props: TagCreatorProps) {
    ...
    return (
        <div>
            <Tooltip2 content="Create subject specific tags. Type the tag and hit enter, then sign the transaction">
                <TagInput
                    fill={true}
